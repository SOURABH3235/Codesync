...
""")
    ...
    public List<Project> getProjects(String ownerEmail) {
        return projectRepository.findByOwnerEmailOrSharedWith(ownerEmail);
    }
    ...
    // 2. Add the new share logic
    @Override
    public void shareProject(Long projectId, String collaboratorEmail) {
        Project project = projectRepository.findById(projectId)
                .orElseThrow(() -> new RuntimeException("Project not found"));

        project.getSharedWithEmails().add(collaboratorEmail);
        projectRepository.save(project);
    }

    @Override
    public List<Project> getProjectsByUser(String email) {
        return projectRepository.findByOwnerEmailOrSharedWith(email);
    }
}